...
function [hFig] = plot_stability_bfo90_table_percell(analysisSettings, analysisResults, DaysToSessions, STABILITY_THRESHOLD_CRITERIA)
    
    [T] = compute_stability_table(analysisSettings, analysisResults)
    
    % Makes a figure with 4 subplots each with a BFO 90
    
    SessionsEarly = DaysToSessions(ismember(DaysToSessions.dayNum, [1,2]),:);
    MatchEarly = get_match_table_by_sessions(SessionsEarly, StabilityBFO90);
    REarly = StabilityBFO90(MatchEarly.TableBRowIndex,:);

    SessionsLate = DaysToSessions(ismember(DaysToSessions.dayNum, [3,4]), :);
    MatchLate = get_match_table_by_sessions(SessionsLate, StabilityBFO90);
    RLate = StabilityBFO90(MatchLate.TableBRowIndex,:);
    ...
    ax = [];
    ax(1) = subplot(2,2,1);
    barplot_with_errors_selected(REarly, {'unstable_within', 'unstable_across'}, colours)
    title('Early Training')
    a = axis;
    axis([a(1) a(2) 0, maxY])

    ax(2) = subplot(2,2,2);
    barplot_with_errors_selected(RLate, {'unstable_within', 'unstable_across'}, colours)
    title('Late Training')
    a = axis;
    axis([a(1) a(2) 0, maxY])

    ax(3) = subplot(2,2,3);
    barplot_with_errors_selected(REarly, {'stable_within', 'stable_across'}, colours)
    title('Early Training')
    a = axis;
    axis([a(1) a(2) 0, maxY])

    ax(4) = subplot(2,2,4);
    barplot_with_errors_selected(RLate, {'stable_within', 'stable_across'}, colours)
    title('Late Training')
    a = axis;
    axis([a(1) a(2) 0, maxY])

    sgtitle(sprintf('Stable/Unstable Threshold = %0.2f', STABILITY_THRESHOLD_CRITERIA), 'interpreter', 'none');
end
