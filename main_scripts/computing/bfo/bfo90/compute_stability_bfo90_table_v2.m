...
function [R, success] = compute_stability_bfo90_table_v2(StabilityPerCell, BFO90_FILTER_MIN_MAPS, BFO90_FILTER_MIN_TRIAL_CORRELATION)
    % This differs from version 1 in that the stability information comes
    % from the percell data, not the best aligned.
...
%     PerCellMatched.stable = PerCellMatched.isStable;
    ...
    SessionTable = get_sessions_from_table(StabilityPerCell);
    ...
    for iSession = 1:numSessions
        ...
        PerCellMatchedSession = StabilityPerCell(ismember(StabilityPerCell.animalName, animalName) & ismember(StabilityPerCell.sessionName, sessionName), :);

        % Unstable
        for iField = 1:length(fieldSets)
            fieldNamesWanted = fieldSets(iField).fieldNames;

            [hc_group, prob_group, totalSamples, numCells, numSamplesExcluded] = compute_bfo_using_percell_table(PerCellMatchedSession(PerCellMatchedSession.isStable==false,:), BFO90_ROTATIONS_TO_CHECK, fieldNamesWanted, BFO90_FILTER_MIN_MAPS, BFO90_FILTER_MIN_TRIAL_CORRELATION);
        ...
        for iField = 1:length(fieldSets)
            ...
            [hc_group, prob_group, totalSamples, numCells, numSamplesExcluded] = compute_bfo_using_percell_table(PerCellMatchedSession(PerCellMatchedSession.isStable==true,:), BFO90_ROTATIONS_TO_CHECK, fieldNamesWanted, BFO90_FILTER_MIN_MAPS, BFO90_FILTER_MIN_TRIAL_CORRELATION);
