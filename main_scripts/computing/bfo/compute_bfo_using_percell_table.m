...
function [hc_group, prob_group, totalSamples, numCells, numSamplesExcluded] = compute_bfo_using_percell_table(PerCell, rotationsDeg, fieldNamesWanted, BFO90_MIN_MAPS, BF90_MIN_CORRELATION)
    ...
    numCells = size(PerCell,1);

    % Compute minimum number of comparisons based on the minimum number of
    % trials.
    MIN_COMPARISONS = BFO90_MIN_MAPS*(BFO90_MIN_MAPS-1)/2;
    ...
    for iCell = 1:numCells
       ...
       for iFieldName = 1:numFieldNames
            ...
            % The angle index
            vind_cell = PerCell.(sprintf('vind_%s', fieldName)){iCell};
            % The correlation for the angle
            v_cell = PerCell.(sprintf('v_%s', fieldName)){iCell};
            ...
            if ~isempty(BF90_MIN_CORRELATION)
                vind_use = vind_cell(v_cell >= BF90_MIN_CORRELATION);
                v_use = v_cell(v_cell >= BF90_MIN_CORRELATION);
                
                not_used_vinds = cat(1, not_used_vinds, vind_cell(v_cell < BF90_MIN_CORRELATION));
                not_used_vs = cat(1, not_used_vs, v_cell(v_cell < BF90_MIN_CORRELATION));
            else
    ...
    hc_group = histcounts(best_inds, 1:(length(rotationsDeg)+1));
    
    totalSamples = length(best_inds);
    
    s = sum(hc_group, 'all');
    if s ~= 0 
        prob_group = hc_group ./ s * 100;
    else
        prob_group = zeros(1, length(rotationsDeg));
    end
