...
function compute_popvectors_correlations_within_contexts(analysisSettings, CellRegTable, BestAligned, BESTALIGNED_STABILITY_THRESHOLD_CRITERIA, STABILITY_CLASSIFICATION_GROUP_LABEL)
...
% stable and unstable registered cell names.
...
plot(popvectorResults.stable.dp_across_context_1.uz, popvectorResults.stable.dp_across_context_1.cz, 'b-', 'linewidth', LW)
...
plot(popvectorResults.stable.dp_across_context_2.uz, popvectorResults.stable.dp_across_context_2.cz, 'b--', 'linewidth', LW)
...
plot(popvectorResults.unstable.dp_across_context_1.uz, popvectorResults.unstable.dp_across_context_1.cz, 'r-', 'linewidth', LW)
...
plot(popvectorResults.unstable.dp_across_context_2.uz, popvectorResults.unstable.dp_across_context_2.cz, 'r--', 'linewidth', LW)
...
[ks1,p1] = kstest2(FS.('dp_across_context_1'){1},FS.('dp_across_context_2'){1});
[ks2,p2] = kstest2(FU.('dp_across_context_1'){1},FU.('dp_across_context_2'){1});

% These compare stable with unstable
[ks3,p3] = kstest2(FS.('dp_across_context_1'){1},FU.('dp_across_context_1'){1});
[ks4,p4] = kstest2(FS.('dp_across_context_2'){1},FU.('dp_across_context_2'){1});
[ks5,p5] = kstest2(FS.('dp_across_context_1'){1},FU.('dp_across_context_2'){1});
[ks6,p6] = kstest2(FS.('dp_across_context_2'){1},FU.('dp_across_context_1'){1});


legend({...
    sprintf('Stable (on %s) context 1 (%s) with context 1 (%s)',STABILITY_CLASSIFICATION_GROUP_LABEL, sessionNameA, sessionNameB), ...
    sprintf('Stable (on %s) context 2 (%s) with context 2 (%s)',STABILITY_CLASSIFICATION_GROUP_LABEL, sessionNameA, sessionNameB), ...
    sprintf('Unstable (on %s) context 1 (%s) with context 1 (%s)',STABILITY_CLASSIFICATION_GROUP_LABEL, sessionNameA, sessionNameB), ...
    sprintf('Unstable (on %s) context 2 (%s) with context 2 (%s)',STABILITY_CLASSIFICATION_GROUP_LABEL, sessionNameA, sessionNameB)}, ...
...
xlabel('Popvectors Correlations', 'fontsize', FSz)
ylabel('Cumulative', 'fontsize', FSz)

% save the output to here
outputFolder = fullfile(analysisSettings.OUTPUT_RESULTS_FOLDER, 'popvectors_correlations');
if ~exist(outputFolder, 'dir')
    mkdir(outputFolder);
end

% save the figure
mulana_savefig(hFig, outputFolder, sprintf('calcium_popvectors_correlations_within_context_classified_%s',STABILITY_CLASSIFICATION_GROUP_LABEL), {'png', 'svg', 'fig'});

% Save the data
outputFilename = fullfile(outputFolder, sprintf('calciumPopvectorCorrelationsResults_within_context_%s.mat',STABILITY_CLASSIFICATION_GROUP_LABEL) );
save(outputFilename, 'popvectorResults', 'analysisSettings', ...
    ...
    'p1', 'p2', 'p3', 'p4', 'p5', 'p6');
fprintf('popvectorCorrelationsWithinResults.mat saved to %s\n', outputFilename);
...
function [F] = compute_F(BestAligned)
    ...
    for iA = 1:numDays
        ...
        for iB = iA+1:numDays
            ...
            maps1 = helper_map_cells_to_3dmatrix(T.mapDayAContext1);
            maps2 = helper_map_cells_to_3dmatrix(T.mapDayAContext2);
            [dp, dpAvg] = ml_alg_popvectors_compute_correlations(maps1, maps2);
