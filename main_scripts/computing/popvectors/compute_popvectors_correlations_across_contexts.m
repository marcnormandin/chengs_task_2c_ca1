...
function compute_popvectors_correlations_across_contexts(analysisSettings, CellRegTable, BestAligned, BESTALIGNED_STABILITY_THRESHOLD_CRITERIA, STABILITY_CLASSIFICATION_GROUP_LABEL)
...
xlabel('Popvector DP Correlation', 'fontsize', FSz)
...
% save the figure
mulana_savefig(hFig, outputFolder, sprintf('calcium_popvectors_correlations_across_context_classified_%s',STABILITY_CLASSIFICATION_GROUP_LABEL), {'png', 'svg', 'fig'});

% Save the data
outputFilename = fullfile(outputFolder, sprintf('calciumPopvectorCorrelationsResults_across_context_%s.mat',STABILITY_CLASSIFICATION_GROUP_LABEL) );
save(outputFilename, 'popvectorResults', 'analysisSettings', ...
    ...
    'p1', 'p2', 'p3', 'p4', 'p5', 'p6');
fprintf('popvectorCorrelationsAcrossResults.mat saved to %s\n', outputFilename);
