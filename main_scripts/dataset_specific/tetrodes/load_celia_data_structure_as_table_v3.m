...
function [T] = load_celia_data_structure_as_table_v3(data)
        ...
        for iSession = 1:length(sessionNames)
            ...
            for iTrial = 1:numTrials
                ...
                for iCell = 1:numCells
                   ...
                   if isfield(tdata, 'spk_t')
                       T(k).spk_t = tdata.spk_t{iCell};
                   end
                   
                   if isfield(tdata, 'spk_x')
                       T(k).spk_x = tdata.spk_x{iCell};
                   end
                   
                   if isfield(tdata, 'spk_y')
                       T(k).spk_y = tdata.spk_y{iCell};
                   end
                   ...
                   if isfield(tdata, 't_sec')
                       T(k).t_sec = tdata.t_sec;
                   end
                   
                   if isfield(tdata, 'runSpeed')
                       T(k).runSpeed = tdata.runSpeed;
                   end
