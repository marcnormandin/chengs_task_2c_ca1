...
% to plot them manually. Makes Figure S15B (pfr).
...
MAIN_FOLDER = fullfile('.\local_output','pfrs');
...
for i = 1:length(FRACTIONS)
    ...
    fnList{i,1} = fraction;
    fnList{i,2} = fullfile(MAIN_FOLDER, sprintf('min_fraction_occupied_%0.1f', fraction), sprintf('tetrodes_rate_matrix_within_across_per_cell_pfrs_min_fraction_occupied_%0.1f.xlsx', fraction));
end
