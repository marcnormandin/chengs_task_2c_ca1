...
% This makes the Excel files required for Figure S13B
close all
clear all
clc
...
for iDataset = 1:length(datasets)
    dataset = datasets{iDataset};
    T = dataset.T;
    E = T(:, ismember(T.Properties.VariableNames, {'animalName', 'sessionName', 'animalSessionCellName', 'isStable', 'groupId', 'groupLabel', 'ics_c1_avg', 'ics_c2_avg', 'ics_avg'}));
    E =  sortrows(E, {'isStable', 'groupLabel', 'animalName'});

    if dataset.analysisSettings.IS_CALCIUM_DATA
        outputFn = fullfile(OUTPUT_FOLDER, 'information_content_calcium_data.xlsx');
    else
        outputFn = fullfile(OUTPUT_FOLDER, 'information_content_tetrodes_data.xlsx');
    end
...
%% FUNCTIONS
function [T] = compute_and_plot(dataset)
...
function [T] = compute_avg_ics_per_cell(analysisSettings, MapsData, BestAligned)
    ...
    for bestAligned_row_index = 1:size(BestAligned,1)
        ...
        %[informationRate, informationPerSpike] = ml_placefield_informationcontent_v2(meanFiringRateMap, positionProbMap)
...
% population vector code so we can use the same code.
            ...
            for iTrial = 1:numTrials
                ...
                for iCell = 1:numCells
                   ...
                   if RECALC_INFORMATION
                       ...
                       % fieldnames as the normal ratemaps structure for rectangular
